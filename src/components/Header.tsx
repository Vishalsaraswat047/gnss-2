...
import { Satellite, Layers, Database, Radio, Info } from 'lucide-react';

export type AppTab = 'dashboard' | 'data' | 'live';
    ...
    <header className="bg-[#0A0C10] border-b border-slate-800 sticky top-0 z-40">
      ...
      <div className="bg-slate-950/80 border-b border-slate-700/60 px-4 py-1.5 text-center text-[10px] text-slate-500 flex items-center justify-center gap-1">
        ...
        <span>
          ...
          <span className="hidden sm:inline"> • XGBoost • SAT-01 • 7 days → Day 8 (96×15 min)</span>
        </span>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-3 pb-2.5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`ml-2 px-2 py-0.5 rounded text-[10px] font-bold border ${riskLevel==='HIGH' ? 'bg-amber-500/20 text-amber-400 border-amber-500/40' : riskLevel==='MEDIUM' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' : 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30'}`}>{riskLevel}</span>
              </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 bg-slate-900/60 border border-slate-800 rounded-lg px-2 py-1.5">
